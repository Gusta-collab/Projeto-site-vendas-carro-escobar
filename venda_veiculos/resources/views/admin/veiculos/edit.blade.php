            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ route('admin.veiculos.update', $veiculo->id) }}">
                        @csrf
                        @method('PUT') {{-- MUDANÇA 2: Método PUT para edição --}}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
                            ...
                            <div>
                                ...
                                <select name="cor_id" id="cor_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                                    ...
                                    @foreach ($cores as $cor)
                                        {{-- MUDANÇA 3: 'selected' para a cor atual --}}
                            ...
                            <div>
                                ...
                                {{-- MUDANÇA 3: 'value' com o ano atual --}}
                                <input type="number" name="ano" id="ano" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required value="{{ old('ano', $veiculo->ano) }}">
                            </div>

                            <div>
                                <label for="quilometragem" class="block text-sm font-medium text-gray-700">Quilometragem (KM)</label>
                                {{-- MUDANÇA 3: 'value' com o km atual --}}
                                <input type="number" name="quilometragem" id="quilometragem" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required value="{{ old('quilometragem', $veiculo->quilometragem) }}">
                            </div>
                        </div>

                        <div class="mb-4">
                            <label for="valor" class="block text-sm font-medium text-gray-700">Valor (R$)</label>
                            {{-- MUDANÇA 3: 'value' com o valor atual --}}
                            <input type="number" name="valor" id="valor" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required step="0.01" value="{{ old('valor', $veiculo->valor) }}">
                        </div>

                        <div class="mb-4">
                            <label for="detalhes" class="block text-sm font-medium text-gray-700">Detalhes (Descrição)</label>
                            {{-- MUDANÇA 3: 'value' (texto) com os detalhes atuais --}}
                            <textarea name="detalhes" id="detalhes" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">{{ old('detalhes', $veiculo->detalhes) }}</textarea>
                        </div>
                        
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
                            <div>
                                <label for="foto_1" class="block text-sm font-medium text-gray-700">Foto 1 (Principal - URL)</label>
                                {{-- MUDANÇA 3: 'value' com a foto atual --}}
                                <input type="text" name="foto_1" id="foto_1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('foto_1', $veiculo->foto_1) }}">
                            </div>
                            <div>
                                <label for="foto_2" class="block text-sm font-medium text-gray-700">Foto 2 (URL)</label>
                                <input type="text" name="foto_2" id="foto_2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('foto_2', $veiculo->foto_2) }}">
                            </div>
                            <div>
                                <label for="foto_3" class="block text-sm font-medium text-gray-700">Foto 3 (URL)</label>
                                <input type="text" name="foto_3" id="foto_3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" value="{{ old('foto_3', $veiculo->foto_3) }}">
                            </div>
                        </div>
